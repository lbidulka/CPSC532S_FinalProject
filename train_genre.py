...
for epoch in range(epochs):
    ...
    # Training
    Classifier.train()
    for batch_idx, batch in enumerate(audioset.genre_unbal_trainloader):
        x = batch[0]
        y = batch[1]
        x = x.to(device)
        y = y.to(device)
    ...
    Classifier.eval()
    with torch.no_grad():
        for batch_idx, batch in enumerate(audioset.genre_trainloader):
            x = batch[0]
            y = batch[1]
...
# Save the model
torch.save(Classifier, "./CPSC532S_FinalProject/checkpoints/genre_classifier.pth")
...
plt.legend()
plt.savefig("./CPSC532S_FinalProject/outputs/train_curves/genre_training_curves.jpg")
plt.show()  
